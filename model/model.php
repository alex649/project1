...
function buy_shares($userid, $symbol, $last_trade, $shares, &$error) 
{
	...
	// delete the old entry in the table
	    ...
	    $query = sprintf("UPDATE `CS75Finance`.`portfolios` SET 
		`id`='$userid', `symbol`='$symbol', `shares`='$num_of_shares', `cost`='$cost' WHERE id='$userid' AND symbol='$symbol'");
            $pdo->query($query);
        }
	else
	{
...
function sell_shares($userid, $symbol, &$error)
{
	...
	$query = sprintf("UPDATE users SET balance='%s' WHERE id='%s'", $new_balance, $userid);
        $pdo->query($query);
    }
    catch (Exception $e)
    {
